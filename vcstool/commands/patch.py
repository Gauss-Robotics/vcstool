...
def make_absolute(path):
    ...
    if not ppath.is_absolute():
        ppath = Path(os.getcwd()) / ppath
    return str(ppath)


class PatchCommand(Command):
    command = "patch"
    help = "Patch repositories"

    def __init__(self, args, url, patch):
        super(PatchCommand, self).__init__(args)
        self.url = url
        ...
        if patch is None:
            ...
            patch = [patch]
        self.patch = list(map(make_absolute, patch))


def get_parser():
    parser = argparse.ArgumentParser(
        description="Patch repositories :)", prog="vcs patch"
    )
    group = parser.add_argument_group('"patch" command parameters')
    group.add_argument(
        "--input",
...
def file_or_url_type(value):
    ...
    return request.Request(
        value,
        headers={"User-Agent": "vcstool/" + vcstool_version}
        )
    ...
    for path, repo in repos.items():
        ...
        if not clients:
            ...
            job = {
                ...
                "command": None,
                "cwd": path,
                "output": f"Repository type '{repo['type']}' is not supported",
        ...
        command = PatchCommand(
            args, repo["url"], repo["patch"] if "patch" in repo else None
        )
